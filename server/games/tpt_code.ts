...
import {
    ...
    OTHER_NAMES,
    OTHER_NAMES_INV
} from '../../client/tpt_code/js/data.js';

// All element code, as ['ANSWER.cpp', 'uncensored code string']
import fs from 'fs';
const CODE = fs.readFileSync('../client/tpt_code/code.txt', { encoding: 'utf8', flag: 'r' })
    .split('-SPECIAL_DIVIDER-')
    .map(x => x.split('+SPECIAL_DIVIDER+'));
...
interface TPTCodeMessage {
    answer: string;
}

class TPTCodeGame extends Game {
    round: number;
    state: TPTState;
    ...
    onMove(client: Client, message: TPTCodeMessage) {
        // Message format:
    ...
    toGuessState() {
        ...
        this.codeIndex = Math.floor(Math.random() * CODE.length);
        while (this.picked.includes(this.codeIndex)) // No duplicates in a round
            this.codeIndex = Math.floor(Math.random() * CODE.length);
        this.picked.push(this.codeIndex);

        this.broadcast({
            type: 'TURN_QUESTION',
            code: CODE[this.codeIndex][1],
            round: this.round,
    ...
    toRevealState() {
        // @ts-ignore-error
        let answer = CODE[this.codeIndex][0].replace('.cpp', '').trim();
