...
connection.onopen = () => {
    if (uuid.length === 0) // Create a new game
        connection.send(JSON.stringify({ type: 'CREATE', gameType: 'tpt_code' }));
...
connection.onmessage = (message: any) => {
    ...
    switch (message.type) {
        ...
        case 'TURN_QUESTION': {
            ...
            (document.getElementById('element-code') as HTMLDivElement).innerHTML =
                // @ts-expect-error
                Prism.highlight(message.code, Prism.languages.clike, 'cpp');
